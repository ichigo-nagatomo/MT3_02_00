...
Vec3 ClosestPoint(const Vec3 &point , const Segment &segment) {
	Vec3 result;
	Vec3 projba = Project(Subtract(point , segment.origin) , segment.diff);
	result.x = segment.origin.x + projba.x;
	result.y = segment.origin.y + projba.y;
	result.z = segment.origin.z + projba.z;
...
const char kWindowTitle[] = "LD2B_06_ナガトモイチゴ_MT3_02_00";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Vec3 start = segment.origin;
	Vec3 end = segment.diff;
	...
	while (Novice::ProcessMessage() == 0) {
		...
		DrawSphere(pointSphere , projectionMatrix , viewportMatrix , RED);
		DrawSphere(closestSphere , projectionMatrix , viewportMatrix , BLACK);
		DrawLine(segment , projectionMatrix , viewportMatrix , WHITE);
